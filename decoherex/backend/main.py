...
# React app dist directory (Vite outputs to "dist")
REACT_DIST = os.path.join(os.path.dirname(__file__), "..", "frontend", "decoherex_analytics", "dist")


@app.get("/")
async def read_root():
    """Serve the frontend - React app if built, else legacy static."""
    if os.path.exists(REACT_DIST) and os.path.isdir(REACT_DIST):
...
async def generate_circuit_code(request: GenerateRequest):
    ...
    if groq_client:
        try:
            ...
            # Define system prompt (Corrected)
            system_context = """You are an expert quantum application developer.
Your goal is to write a valid, executable Qiskit circuit based on the user's request.

Rules:
1. Return ONLY Python code. No markdown (```python ... ```), no explanations.
2. Structure:
   - Imports
   - Circuit definition (e.g. `qc = QuantumCircuit(...)`)
   - Gate operations
   - Measurements (unless asked otherwise)
3. Do NOT use `BasicAer`, `execute`, or deprecated modules.
4. Use `QuantumCircuit(num_qubits)` directly.

If the user asks for a specific algorithm (GHZ, Teleportation, QFT), implement it correctly.
"""

            completion = groq_client.chat.completions.create(
                messages=[
                    {"role": "system", "content": system_context},
                    {"role": "user", "content": f"Write a Qiskit circuit for: {request.prompt}"}
                ],
                model=model_name,
                temperature=0.3,
            )
            ...
            # Do NOT silently fallback, perform a final explicit check or return error code
            # But to keep user experience smooth, we will use the fallback but LOG it clearly
            # and potentially prepend a comment to the code
            pass

    # Fallback Rule-Based Logic
    fallback_model = "Rule-Based (Fallback)"
    logging.warning("Using Rule-Based Fallback for Circuit Generation")
    
    # Simple default if no keywords match
    default_code = """from qiskit import QuantumCircuit
# AI Generation Failed - Defaulting to Bell State
qc = QuantumCircuit(2)
qc.h(0)
qc.cx(0, 1)
qc.measure_all()"""
...
qc.measure_all()"""
    ...
    return {
        ...
        "note": "AI generation failed, used fallback."
    }
...
async def simulate_circuit(request: SimulateRequest):
    ...
    try:
        ...
        sanitized_code = re.sub(r"from qiskit import execute\b", "", sanitized_code)
        sanitized_code = re.sub(r"from qiskit import Aer\b", "", sanitized_code)
        sanitized_code = re.sub(r"from qiskit import BasicAer\b", "", sanitized_code)
        sanitized_code = re.sub(r"from qiskit\.providers\.aer import \b", "from qiskit_aer import ", sanitized_code)
        sanitized_code = re.sub(r"from qiskit\.providers\.basicaer import \b", "from qiskit_aer import ", sanitized_code)
        sanitized_code = re.sub(r"from qiskit_aer import Aer\b", "", sanitized_code) # avoid Aer.get_backend failures if they manually changed to qiskit_aer but kept Aer
        
        
        # Patch deprecated usage of RX, RY, RZ imports
        sanitized_code = re.sub(r"from qiskit\.circuit\.library import RX\b", "from qiskit.circuit.library import RXGate as RX", sanitized_code)
        sanitized_code = re.sub(r"from qiskit\.circuit\.library import RY\b", "from qiskit.circuit.library import RYGate as RY", sanitized_code)
        sanitized_code = re.sub(r"from qiskit\.circuit\.library import RZ\b", "from qiskit.circuit.library import RZGate as RZ", sanitized_code)
        sanitized_code = re.sub(r"from qiskit\.circuit\.library import X\b", "from qiskit.circuit.library import XGate as X", sanitized_code)
        sanitized_code = re.sub(r"from qiskit\.circuit\.library import Y\b", "from qiskit.circuit.library import YGate as Y", sanitized_code)
        sanitized_code = re.sub(r"from qiskit\.circuit\.library import Z\b", "from qiskit.circuit.library import ZGate as Z", sanitized_code)
        sanitized_code = re.sub(r"from qiskit\.circuit\.library import H\b", "from qiskit.circuit.library import HGate as H", sanitized_code)
        ...
        logging.info("Executing user circuit code...")
        try:
            exec(sanitized_code, local_scope, local_scope)
        except NameError as ne:
             logging.error(f"Execution NameError: {ne}")
             return {"error": f"Code execution failed: {ne}. Did you forget an import?"}
        except Exception as e:
        ...
        # Let transpile use the simulator's native target (don't pass basis_gates - barrier/reset are not allowed there in Qiskit 1.0+)
        ...
        # 3.5 CALCULATE METRICS (Before robust diagram generation)
        depth = qc.depth()
...
async def explain_circuit(request: ExplainRequest):
    """
    Returns an AI-generated explanation using the user's specific prompt format.
    """
    ...
    if groq_client:
        try:
            ...
            # Construct the system context with metrics
            system_prompt = f"""You are a quantum computing tutor.
...
Circuit details:
- Number of qubits: {request.num_qubits if request.num_qubits is not None else 'Unknown'}
- Gates used: {request.gates if request.gates else 'Unknown'}
- Circuit depth: {request.depth if request.depth is not None else 'Unknown'}

Simulation results:
- Measurement counts: {request.counts if request.counts else 'Not executed'}
- Total shots: {request.shots}

Explain in simple, clear language:
1. What the quantum circuit does
2. What the result chart represents
3. What the simulation results imply
"""
            ...
            # Map simplified response to frontend expectations
            # The frontend expects { summary, description, concepts }
            # The new prompt returns { explanation }
            # We map 'explanation' to 'description' and provide generic summary/concepts
            
            return {
                "summary": "AI Quantum Tutor Analysis",
                "description": data.get("explanation", "No explanation provided."),
                "concepts": ["Analysis", "Simulation Results"], 
            }
            
        except json.JSONDecodeError as e:
            logging.error(f"Explain JSON parse error: {e}")
            return {"summary": "", "description": "Could not parse AI explanation.", "concepts": []}
        except Exception as e:
            logging.error(f"Groq Explain Error: {e}")
            return {"summary": "", "description": f"Explanation unavailable: {str(e)}", "concepts": []}

    return {
        "summary": "AI explanation requires GROQ_API_KEY.",
        "description": "Add GROQ_API_KEY to your backend .env to get circuit explanations.",
        "concepts": [],
    }


# Serve React app static assets and SPA fallback (when dist exists)
if os.path.exists(REACT_DIST) and os.path.isdir(REACT_DIST):
    assets_dir = os.path.join(REACT_DIST, "assets")
    if os.path.exists(assets_dir):
        app.mount("/assets", StaticFiles(directory=assets_dir), name="assets")
    # SPA fallback: serve index.html for client-side routes (e.g. /quantum-lab, /live-jobs)
    @app.get("/{full_path:path}")
    async def serve_spa(full_path: str):
        if full_path.startswith("api/") or full_path.startswith("assets/"):
            raise HTTPException(status_code=404, detail="Not found")
        # Serve static files from dist root (favicon, manifest, etc.)
        static_file = os.path.join(REACT_DIST, full_path)
        ...
        if os.path.exists(index_path):
            return FileResponse(index_path)
        raise HTTPException(status_code=404, detail="Not found")
elif os.path.exists(os.path.join(os.path.dirname(__file__), "..", "frontend", "decoherex_analytics", "build")):
